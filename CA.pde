...
class CA {
	...
	color colorOff = color(255, 89, 89);
	color colorOn = color(140, 202, 255);
	color colorOff2 = color(190, 108, 132);
	color colorOn2 = color(53, 94, 124);

	CA(int[] r, int[] letters) {
		rules = r;
		scl = 8;
	...
	void restart(int[] letters) {
		for (int i = 0; i < cells.length; i++) {
			cells[i] = 0;
		}
		int letterCounter = 0;
		for (int i = (cells.length - 32)/2; i < (cells.length + 32)/2; i++) {
			cells[i] = letters[letterCounter];
			letterCounter++;
		}
		generation = 0;
	}
